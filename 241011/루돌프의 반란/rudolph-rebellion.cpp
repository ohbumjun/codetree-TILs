...
{
	// int initRow = stR + dRow[dir];
	// int initCol = stC + dCol[dir];

	int initRow = stR + addedR * moveCnt;
	int initCol = stC + addedC * moveCnt;

	stataPosMap[stR][stC] = -1;

	// 만약 범위 벗어나면 그냥 out 처리
	if (initRow < 0 || initRow >= N || initCol < 0 || initCol >= N)
	{
		aliveSantas[santa] = false;
		return;
	}

	queue<tuple<int,int, int>> q;
	q.push({initRow, initCol, santa });

	while (!q.empty())
	{
		// q. 에 있는 녀석을
		int curRow, curCol, curSanta;
		tie(curRow, curCol, curSanta) = q.front();
		q.pop();
		...
		{
			// 범위 안
			if (stataPosMap[curRow][curCol] == -1)
			{
				stataPosMap[curRow][curCol] = curSanta;
			...
			else
			{
				...
				//	queue 에 다시 넣는다.
				int oldSanta = stataPosMap[curRow][curCol];
				stataPosMap[curRow][curCol] = curSanta;
				q.push({ curRow + addedR,
					curCol + addedC, oldSanta });
			}
		}
	}
}
